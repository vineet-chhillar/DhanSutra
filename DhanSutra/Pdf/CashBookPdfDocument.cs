...
{
    public class CashBookPdfDocument : IDocument
    {
        private readonly CashBookDto _dto;
        private readonly DateTime _from;
        private readonly DateTime _to;

        public CashBookPdfDocument(
            CashBookDto dto,
            DateTime from,
            DateTime to
        )
        ...
        void ComposeContent(IContainer container)
        {
            container.Column(col =>
            {
                ...
                col.Item().Text(
                    $"Opening Balance : {_dto.OpeningBalance:N2}"
                ).Bold();
                ...
                col.Item().AlignRight().Text(
                    $"Closing Balance : {_dto.ClosingBalance:N2}"
                ).Bold();
            });
        }
